...
import torch
import numpy as np
from tlidb.examples.utils import detach_and_clone, collate_list, concat_t_d, save_algorithm_if_needed, save_pred_if_needed, write_output
...
def run_epoch(algorithm, datasets, config, logger, train):
    ...
    if config.model_type == 'SVM':
        if train:
            algorithm.svm_fit(config, datasets)
        else:
            f1, instance_id_y_pred = algorithm.svm_eval(config, datasets)
            return f1, instance_id_y_pred
    else:

        # with open("../../../outputs/test.txt", "w") as f:
        #     f.writelines('just_testing')
        if train:
            algorithm.train()
        ...
        # add by Yian
        metadata = {t_d: [] for t_d in task_datasets}
        ...
        for batch in pbar:
            ...
            #add by Yian
            metadata[batch_t_d].append(detach_and_clone(batch_results['metadata']['instance_id']))
        ...
        for t_d in task_datasets:
            ...
            # add by Yian
            metadata[t_d] = collate_list(metadata[t_d])
        ...
        if algorithm.requires_metric_calculation():
            ...
            for m, d in zip(datasets['metrics'],datasets['datasets']):
                #add by Yian
                # pdb.set_trace()
                # assert(len(metadata[t_d]) == len(epoch_y_pred[t_d]))
                # with open("../../../outputs/D3/bert_dev.txt", "w") as f:
                #     for i in range(len(metadata[t_d])):
                #         f.writelines(metadata[t_d][i]+',')
                #         f.writelines(str(epoch_y_pred[t_d][i].numpy())+'\n')
...
def train(algorithm, datasets, config, logger, epoch_offset, best_val_metric):
    for epoch in range(epoch_offset, config.num_epochs):
        ...
        val_results, val_y_pred = run_epoch(algorithm, datasets['dev'], config, logger, train=False)
        if config.model_type == 'SVM':
            test_result, y_pred_test= run_epoch(algorithm, datasets['test'], config, logger, train=False)
            micro_f1, weighted_f1 = test_result
            instance_ids, predictions = y_pred_test

            logger.write(f'Test metric: micro_f1={micro_f1:0.4f}, weighted_f1={weighted_f1:0.4f}\n')
            write_output(config.train_tasks[0], config.model, 'test', instance_ids, predictions, logger)

        if isinstance(val_results, tuple):
            micro_f1, weighted_f1 = val_results
            cur_val_metric = micro_f1
            config.save_last = False
            config.save_best = False

            logger.write(f'Validation metric: micro_f1={micro_f1:0.4f}, weighted_f1={weighted_f1:0.4f}\n')
            instance_ids, predictions = val_y_pred
            write_output(config.train_tasks[0], config.model, 'dev', instance_ids, predictions, logger)
        else:
    ...
    for split in datasets:
        for dataset, loader, metric in zip(datasets[split]['datasets'], datasets[split]['loaders'], datasets[split]['metrics']):
            ...
            for batch in pbar:
                ...
                desc = f"{split} losses | {total_loss/loss_divisor:0.4f}"
                pbar.set_description(desc)
            ...
            # add by Yian
            assert(len(epoch_instance_ids) == len(epoch_y_pred))
            label_list =  ["joyful",
          "mad",
          "neutral",
          "peaceful",
          ...
          "sad",
          "scared"]
            with open("../../../outputs/D4/"+dataset.task+"_bert_"+split+".txt", "w") as f:
                for i in range(len(epoch_instance_ids)):
                    f.writelines(epoch_instance_ids[i]+',')
                    f.writelines(label_list[epoch_y_pred[i].numpy()]+'\n')
