...
def write_output(task, model, split, ids, predictions, logger):
    output_file = os.path.abspath(f"../../../outputs/D4/{task}_{model}_{split}.txt")
    logger.write(f"Writing output to {output_file}\n")
    with open(output_file, "w") as f:
        for i in range(len(ids)):
            f.writelines(ids[i]+',')
            f.writelines(str(predictions[i])+'\n')
    logger.write(f"Done writing output to {output_file}\n")